...
@test "first run: creates backup, does not delete when no existing backup" {
    cat > "$MOCK_BIN/oci" << 'EOF'
#!/usr/bin/env bash
echo "$*" >> /tmp/oci_calls.log
if [[ "$*" == *"list"* ]]; then
    echo '{"data": []}'
elif [[ "$*" == *"create"* ]]; then
...
EOF
    ...
    [ "$status" -eq 0 ]
    grep -q "create" /tmp/oci_calls.log
    ! grep -q "delete" /tmp/oci_calls.log
}
...
EOF
    ...
    grep -q "create" /tmp/oci_calls.log
    grep -q "delete" /tmp/oci_calls.log
    grep -q "ocid1.bootvolumebackup.oc1.iad.old001" /tmp/oci_calls.log
}
