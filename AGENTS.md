...
Everything below runs on **docker-compose today**. The lab is migrating to k3s + Terraform + GitOps; that migration is tracked outside this repo, in a private roadmap document. Terraform groundwork has started (`terraform/`, `bpg/proxmox` provider, a cloud-init template on `pve`); one VM (`constrainer`, VMID 104) has been provisioned via Terraform as of #51, but no workloads have migrated onto it yet (#52/#53). Don't assume any Kubernetes tooling exists until a workload actually shows up as manifests in this repo. Debian's cloud images don't ship `qemu-guest-agent`, so `proxmox_virtual_environment_vm` resources need `agent.timeout` set short (not the 15m default) until Ansible installs it — otherwise `plan`/`apply`/`destroy` hang.

## Hosts

- **pve** (`192.168.10.2`) — Proxmox VE, single physical host, 6c/6t. Runs four VMs: **vm100** (`192.168.10.100`), **vm101** (`192.168.40.101`), **pbs** (`192.168.10.103`, Proxmox Backup Server), and **constrainer** (`192.168.10.104`, VMID 104) — vm100's in-progress successor from #51/#52/#53. `pve` is only the SSH/DNS target — its actual Proxmox cluster node name (needed for Terraform's `node_name`) is **pve0**.
